...
2. 高频 schedule 自动取“配置上限”和“调度间隔的 80%”中的较小值，例如 `5m` 为 240 秒、`1m` 为 48 秒、`5s` 为 4 秒；无明确 schedule 时按 `timeframe=Minutes|Seconds` 与 `compression` 推导。该机制不绑定特定市场或固定收盘时间。
...
3. 秒级 provider 必须支持 `timeframe='Seconds'`，网络/SDK 单次超时应明显短于 schedule 周期；24x7 数据请求不得强制使用仅常规交易时段过滤。
