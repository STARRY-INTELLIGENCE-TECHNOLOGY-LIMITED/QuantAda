...
3. 刷新 live data
4. 必需行情刷新失败则跳过本轮并告警；滚动期权候选无 K 线时丢弃该合约并继续
...
## 1.1 配置覆盖的生效规则
1. `config.py` 显式列出各责任域配置模块，并用 `from configs.<name> import *` 将其配置键平铺到统一入口；这不是目录自动扫描，新增责任域需要人工增加一行导入。`configs/manager.py` 仅合并 Broker 连接环境并提供报警状态判断。`run.py --config` 接受入口中当前已平铺的全部大写配置键，用户不需要区分配置来源；未知名称（包括过期别名和拼写错误）会被忽略，但必须明确打印警告。
2. 在命令行解析前就已导入模块的组件，必须在实例化或使用时解析配置默认值，不能通过函数默认参数或导入时常量提前固定。实盘引擎会把生效配置复制到券商运行时快照；数量、账户路由和对账决策必须优先使用该快照。
3. 通过 `getattr(config, ...)` 使用的配置保留所属模块的安全默认值；需要让用户覆盖的键在对应 `configs` 模块中维护，并随该模块的 `import *` 一起平铺，不在 `run.py` 另设重复白名单。
4. 框架核心默认值保留在 `config.py`，责任域默认值保留在 `configs` 包子模块中；根入口只维护固定的责任域导入行，不扫描目录、不猜测模块、不复制默认值。`configs/manager.py` 只维护需要组合的 Broker 环境和报警状态函数。

## 1.2 Futu 事件触发运行
1. `futu_broker` 的 `trigger=subscription` 环境由 Futu 行情 handler 接收事件；回调线程只做时间戳去重和单 worker 派发，完整策略运行仍经过 `LiveTrader.run()` 的刷新、账户/在途快照、风控和下单门控。
2. 事件环境不得同时配置 `schedule`；Futu 没有秒级 K 线订阅，`timeframe='Seconds'` 必须明确拒绝。worker 重叠事件只跳过，不保存待执行意图；worker 失败会释放当前事件键，后续推送可以重试。
...
## 7. 实盘自愈基线
1. 正股/指数，以及仍有持仓、在途或持仓查询失败的期权，其 live data refresh 失败时不执行当轮策略。
2. 在判定跳过前，只对上述必需失败做有限次重试；滚动期权候选无 K 线不触发重试，也不跳过整轮。
...
5. 选股器 `run_selection()` 在实盘 init/数据恢复时调用，构成启动标的池；每根 K 的轮动子集由策略从 `broker.datas` 选择。
6. 在途单与调仓目标的代码匹配只允许已知 IBKR venue 后缀别名；`HK.*` / `SHSE.*` 必须精确匹配。
