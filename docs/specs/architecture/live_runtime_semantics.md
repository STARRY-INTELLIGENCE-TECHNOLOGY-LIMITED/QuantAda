...
## 1.1 配置覆盖的生效规则
1. `config.py` 是统一配置入口，通过显式 import 平铺导出框架核心和各责任域的公共大写名称；`configs/manager.py` 仅合并 Broker 连接环境并提供报警状态判断。`run.py --config` 接受一个 Python 字典，覆盖这套平铺后的名称；用户不需要区分配置来源。仅供模块内部使用的局部参数不应导出到入口。未知名称（包括过期别名和拼写错误）会被忽略，但必须明确打印警告，不能看起来像已经应用的覆盖值。
2. 在命令行解析前就已导入模块的组件，必须在实例化或使用时解析配置默认值，不能通过函数默认参数或导入时常量提前固定。实盘引擎会把生效配置复制到券商运行时快照；数量、账户路由和对账决策必须优先使用该快照。
3. 通过 `getattr(config, ...)` 使用的局部配置保留安全的本地默认值；除非它们经过评估后成为稳定、跨模块的公开配置，否则不应扩充 `config.py` 或加入 CLI 覆盖入口。
4. 框架核心默认值保留在 `config.py`，Broker/Provider/Alarm 等责任域默认值存放在 `configs` 包的子模块中，由 `config.py` 显式导入后统一导出，不复制默认值。`configs/manager.py` 只维护需要组合的 Broker 环境和报警状态函数。单个 adapter/provider/alarm 的局部选项、一次性运维参数和兼容别名仍应留在所属模块的安全默认值中，不能仅为了让 `--config` 可用而公开。
