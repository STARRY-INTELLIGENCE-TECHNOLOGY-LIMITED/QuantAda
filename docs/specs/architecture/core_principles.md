...
5. 实盘容错只能使用本轮内短生命周期的对账/健康标记；这些标记用于判断柜台快照是否可信，不得保存或重放交易意图。
...
5. 实盘专属逻辑不得干扰回测：卖单等待、滚动买入、现金结算等待、broker 同步、实时 pending 查询只适用于 live broker；回测假定订单按计划同步成交，必须保持快速流畅。
6. 新增执行链路必须在调用边界明确 live/backtest 分离：live 可在本轮内有限等待、对账和自愈；回测与优化不得通过 sleep、实时查询、broker 同步或健康标记模拟实盘。
