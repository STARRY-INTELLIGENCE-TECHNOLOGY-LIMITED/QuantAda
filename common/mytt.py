# https://github.com/mpquant/MyTT/blob/main/MyTT.py
...
# ------------------ 0级：核心工具函数 --------------------------------------------
# 废除RD原因：在技术分析和量化交易中，只应在最终展示或记录日志时进行四舍五入。所有的中间计算和逻辑判断都必须使用最高精度的原始数据。
...
def FILTER(S, N):
    S_copy = np.copy(np.array(S))  # 创建一个副本
...
def CROSS(S1, S2):
    # 将输入转换为 Pandas Series 以便使用 shift 操作
    S1 = pd.Series(S1)
    S2 = pd.Series(S2)

    # 判断金叉条件
    # np.logical_and 用于确保布尔运算的正确性，并返回 numpy 数组
    return np.array(np.logical_and(S1 > S2, S1.shift(1) < S2.shift(1)), dtype=bool)
