"""现金担保卖出 Put 样例：单腿 SELL_TO_OPEN，必须带风险腿。

复制执行。密钥只放环境变量 FUTU_UNLOCK_PASSWORD，不要写进本文件。
`--params` 与类默认参数一致；平仓调试把 defense_dte 调到大于当前 DTE。

开仓:
python run.py strategies.options.sample_cash_secured_put_strategy.SampleCashSecuredPutStrategy \\
...
class SampleCashSecuredPutStrategy(BaseStrategy):
    """用现金担保卖出一张 Put；裸卖会被 Broker 拒绝。"""
    ...
    def next(self):
        ...
        print(
            "[SampleCashSecuredPutStrategy] occupied=%s candidates=%s signals=%s"
        ...
        for item in sorted(candidates, key=lambda value: value["score"]):
            ...
            order = self._submit(
                ...
                risk_leg=risk_leg,
                underlying_positions={getattr(underlying, "_name", key): position_size(self.broker, underlying)},
            )
            signal = {
                "symbol": item["meta"]["symbol"],
                "action": "SELL_TO_OPEN",
            ...
            self.last_signals.append(signal)
            print("[SampleCashSecuredPutStrategy] %s" % signal)
