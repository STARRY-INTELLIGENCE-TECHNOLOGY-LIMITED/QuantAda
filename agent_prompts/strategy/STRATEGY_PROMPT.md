...
4. **交易池遍历约定**
   - 轮动选股、排名和交易决策优先遍历 `self.broker.datas`，它就是当前策略
     加载的标的池。
   - 账户中未加载进 `self.broker.datas` 的持仓默认不属于本策略管理范围；
     不要把标的池外对象加入 `target_symbols`。
   - 若目标仅与池内标的 venue 后缀不同，框架会保留兼容映射、推送 WARNING 级
     IM 并继续执行当前计划；这是有意的离席运行容错，不应改为静默中止整轮。
