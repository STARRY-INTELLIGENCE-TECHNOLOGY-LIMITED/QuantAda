...
25. 优化器 MainEval 主回测至少覆盖最近 3 年；如果训练+测试逻辑窗口更长，则覆盖完整训练+测试逻辑窗口。MainEval 与年度固定窗口应优先复用本轮已加载的 `raw_datas` 切片，切片前须把 tz-aware 索引转到 naive UTC；只有 MainEval 初始请求窗口确实未覆盖目标窗口时才允许补拉数据。窗口对齐应周期性打印进度，不为期权末根早于窗口结束逐条刷屏。
